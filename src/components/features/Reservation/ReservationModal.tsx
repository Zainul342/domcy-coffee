...
export const ReservationModal = ({ isOpen, onClose, defaultEvent }: ReservationModalProps) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const message = `Halo Domcy! Saya ingin reservasi meja:%0A` +
                    ...
                    >
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-6">
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-xs font-sans uppercase tracking-wider text-domcy-cream/60 mb-1.5">Your Name</label>
                                    <input
                                        type="text"
                                        required
                                ...
                                <div>
                                    <label className="block text-xs font-sans uppercase tracking-wider text-domcy-cream/60 mb-1.5">WhatsApp Number</label>
                                    <input
                                        type="tel"
                                        required
                                        className="w-full bg-white/5 border border-domcy-cream/20 rounded-lg px-4 py-3 text-domcy-cream placeholder-white/20 focus:outline-none focus:border-domcy-accent transition-colors font-sans"
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-xs font-sans uppercase tracking-wider text-domcy-cream/60 mb-1.5">Date</label>
                                ...
                                <div>
                                    <label className="block text-xs font-sans uppercase tracking-wider text-domcy-cream/60 mb-1.5">Time</label>
                                    <div className="relative">
                                        ...
                                        >
                                            <option value="" className="bg-domcy-green">Select Time</option>
                                            {timeSlots.map(t => (
                                                <option key={t} value={t} className="bg-domcy-green text-domcy-cream">{t}</option>
                                            ))}
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div>
                                <label className="block text-xs font-sans uppercase tracking-wider text-domcy-cream/60 mb-2">Pax (People)</label>
                                <div className="flex gap-2">
                                    {paxOptions.map(pax => (
                                        <button
                                            ...
                                            className={`flex-1 py-2 rounded-lg text-sm font-bold border transition-all ${formData.pax === pax
                            ...
                            <div className="flex items-center gap-3 bg-white/5 p-3 rounded-lg border border-domcy-cream/10">
                                ...
                                />
                                <label htmlFor="live-music" className="text-sm text-domcy-cream font-sans cursor-pointer select-none flex-1">
                                    Reserve for Live Music
                                </label>
                                <Music className={`w-4 h-4 ${formData.isLiveEvent ? 'text-domcy-accent' : 'text-domcy-cream/20'}`} />
                            </div>
