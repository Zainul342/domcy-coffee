...
export const LiveMusicSection = () => {
    ...
    const [selectedEvent, setSelectedEvent] = useState<{ name: string, date: string, time: string } | null>(null);
                        ...
                        >
                            ...
                            <div className="h-64 overflow-hidden relative">
                                ...
                                <div className="absolute top-4 left-4 z-20 bg-domcy-green/90 backdrop-blur border border-domcy-cream/20 px-3 py-1 rounded text-center">
                                    <span className="block font-display text-xl leading-none">{event.date.split(' ')[0]}</span>
                                    <span className="block font-sans text-xs uppercase opacity-80">{event.date.split(' ')[1]}</span>
                                </div>
                            </div>
                            ...
                            <div className="p-6 relative z-20 -mt-12">
                                ...
                                <div className="flex items-center gap-4 text-sm opacity-70 mb-6 font-sans">
                                    <span className="flex items-center gap-1"><Calendar className="w-4 h-4" /> {event.day}</span>
